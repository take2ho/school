...
{   	// ADC1: PA1(pin 41)
	...
	ADC1->CR2 |= (0x0C<<24);	// ext trigger source tim5 cc3 event
...
// PA2을 출력설정하고 Alternate function(TIM5_CH3)으로 사용 선언 
	...
	TIM5->ARR = 50000;	// Auto reload  : 10us * 50K = 500ms(period)
	...
	TIM5->CCER &= ~(0xF<<9);	// CC3P=0: CC3 channel Output Polarity (OCPolarity_High : OC3으로 반전없이 출력)  

	// 'Mode' Selection : Output mode, toggle  
	TIM5->CCMR2 &= ~(0xF<<0); // CC3S(CC3 channel) = '0b00' : Output 
