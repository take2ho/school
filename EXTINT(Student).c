...
void _EXTI_Init(void)
{
	...
	SYSCFG->EXTICR[3] |= 0x7000;        //  +exti15 gpioH로
        
	EXTI->FTSR |= 0x00000100;		// Falling Trigger Enable  (EXTI8:PH8) sw누를때 발생
	EXTI->RTSR |= 0x00000200;		// Rising Trigger  Enable  (EXTI9:PH9)  sw 뗄때 발생
        EXTI->FTSR |= 0x00008000;              // 15번 라이징
    	EXTI->IMR  |= 0x00000300;  	// EXTI8,9 인터럽트 mask (Interrupt Enable)  +15번
        EXTI->IMR  |= 0x00008000;
		
	NVIC->ISER[0] |= (1<<23);   // Enable Interrupt EXTI8,9 Vector table Position 참조  0~31 ISER0, 32~63I SER1......
        NVIC->ISER[1] |= (1<<(40-23));    // exti 15 enable
// 0x00800000
}
