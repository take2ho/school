...
void CalculateAxisPosition(typeDcMotor *control)
{
   control->AxisPosition =  (float)control->Encoder32 /WHEEL_RESOLUTION;
}
void CalculateRPS(typeDcMotor *control)
{
   control->rps = (float)control->Encoder16Diff*Fsample/WHEEL_RESOLUTION;
}
void CalculateRPM(typeDcMotor *control)
{
   control->rpm = (float)control->Encoder16Diff*Fsample/WHEEL_RESOLUTION*60;
}
...
int16_t FloatToInt16Limit(float f, int limit) 
{
   int16_t u;
   if( f >=0){
      u=DcMotor1.f >=U_MAX ?  U_MAX : (int16_t)(DcMotor1.f  + 0.5 ) ; 
   }else{
      u=DcMotor1.f <=-U_MAX ?  -U_MAX :(int16_t)(DcMotor1.f  - 0.5 ) ; 
   }
   return u;
...
int16_t AvoidDeadzone(int16_t u, int16_t requirement)
{
   int16_t result;
   if(u>0)  result = MAX(u, requirement);
   else if(u<0) result = MIN(u, -requirement);
   else result = 0;
   return result;
}