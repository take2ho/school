...
int main(void)
{
    ...
    GPIOG->ODR |= 0x0000; 		// GPIOG->ODR.0 'H'(LED ON)
...
//	}
	...
	if((GPIOH->IDR & 0x4000) == 0)	// SW6를 눌렸을 때(SW6 = LOW) (GPIOH->IDR.9 = 'L' ?) 
	{
		GPIOG->ODR ^=  0x0040;	// LED6 On (GPIOG->ODR.1 'H')
	}
	//else
	//{
	//	GPIOG->ODR &= ~ 0x0040;	// LED6 Off (GPIOG->ODR.1 'L')
//	}
        
        //GPIO Example(3) SW2입력으로  LED2 1초후 Off
  	if((GPIOH->IDR & 0x0003) == 0)	// SW2를 눌렸을 때(SW2 = LOW) (GPIOH->IDR.10 = 'L' ?) 
	{
		GPIOG->ODR |=  0x0003;	// LED2 On (GPIOG->ODR.2 'H')
		DelayMS(1000);		// Delay (1000ms  = 1s)
		GPIOG->ODR &= ~0x0003;	// LED2 Off (GPIOG->ODR.2 'L') 
	}
...
void _GPIO_Init(void)
{
	...
	GPIOH->PUPDR 	&= ~0x55AA0000;	// GPIOH 8~15 : Floating input (No Pull-up, pull-down) :reset state
}	
